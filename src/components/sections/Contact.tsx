...
export default function Contact() {
  ...
  const buildWhatsAppMessage = () => {
    const lines = [
      "*New inquiry from SEPL website*",
      "",
      `*Name:* ${formState.name}`,
      `*Phone:* ${formState.phone}`,
      `*Subject:* ${formState.subject}`,
    ];
    if (formState.email) lines.push(`*Email:* ${formState.email}`);
    if (formState.address) lines.push(`*Address:* ${formState.address}`);
    if (formState.message) lines.push("", "*Message:*", formState.message);
    return lines.join("\n");
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.open(
      `https://wa.me/91${siteConfig.contact.whatsapp}?text=${text}`,
      "_blank",
      "noopener,noreferrer"
    );
    setSubmitted(true);
          ...
          <AnimatedSection animation="slide-left" className="lg:col-span-3">
            ...
            >
              ...
              <p className="mt-1 text-sm text-dark/50">
                Fill in the details and send your message via WhatsApp.
              </p>
              ...
              <div className="mt-5">
                <label htmlFor="address" className="mb-1.5 block text-sm font-medium text-dark/70">
                  Address
                </label>
                <input
                  id="address"
